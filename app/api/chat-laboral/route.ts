...
// Inicializar OpenAI solo si hay una API key disponible
...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      if (!openai) {
        console.warn("OpenAI API no está configurada. Usando respuesta predefinida.")
        return NextResponse.json({
          response:
            "El servicio de consultas laborales no está disponible en este momento. Por favor, intenta más tarde.",
...
${contextoNormativo}`,
      ...
      return NextResponse.json({
        response: aiResponse || RESPUESTAS.DEFAULT,
        userId: userId || "anonymous-user",
      })
    } catch (openaiError) {
